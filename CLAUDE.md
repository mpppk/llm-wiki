...
- **署名はただである。** §5 の Infobox 捏造行も、§12 lint の「育ちすぎたハブ」も起こさない。
  `yuki` はプロジェクトメンバーなので、ページを作らなくてもアイコンは表示される。
...
| 孤立ページ | `cosense listPages <projectUrl> --sort linked` の末尾（`linked: 0`）。入口ページ `[このwikiについて]`、`[log]` の日付ページ、操作ページ `[ingest]` `[query]` `[lint]` は被リンクを持たないのが正常なので除く |
