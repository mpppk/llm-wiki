...
- 極端に長い原文は章単位で分割する。連番は末尾に付ける。
  例: `📄<原題> (1/3)`。1 ページはおよそ 15,000 文字までに収める。
  それを超えると `previewEdit` が `request entity too large` で 400 を返す。
...
**画像は拡張子で終わる URL を使う。** CDN のリサイズエンドポイント
（`/_image?href=...&w=1430&f=webp` のような形）は拡張子がクエリの中にあるため、
画像と認識されず表示されない。多くの場合 `href` パラメータに元画像の URL が入っているので、
URL デコードして取り出す。クエリパラメータが落ちる分、リンク切れにも強くなる。
